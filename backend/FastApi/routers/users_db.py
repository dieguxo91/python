...
# Mongo crea las id como object id, tener en cuenta
# Users DB API
...
async def users():
    return users_schema(db_client.local.users.find()) # creamos esta funcion en el schema para obtener una lista
...
async def post_user(user: User):
    if type(search_user("name",user.name)) == User :
        ...
        id = db_client.local.users.insert_one(user_dict).inserted_id # como no conoce los ficheros tiene que ponerse manual (local e users)
        
        new_user = user_schema(db_client.local.users.find_one({"_id":id})) # Mongodb nos crea el campo _id no id, tener en cuenta
...
async def put_user(user: User):
    ...
    try:
        ...
        db_client.local.users.find_one_and_replace({"_id":ObjectId(user.id)}, user_dict) # buscar por id y reemplazar
    except:    
